...
"%PREFIX%\Library\miktex\texmfs\install\miktex\bin\x64\initexmf.exe" --update-fndb --quiet
...
"%PREFIX%\Library\miktex\texmfs\install\miktex\bin\x64\mpm.exe" --update-db --quiet >NUL 2>&1
for %%x in (adjustbox booktabs collectbox fancyvrb ifoddpage mptopdf ucs url caption xcolor upquote ulem mathpazo) do (
	"%PREFIX%\Library\miktex\texmfs\install\miktex\bin\x64\mpm.exe" --quiet --install %%x >NUL 2>&1
)
